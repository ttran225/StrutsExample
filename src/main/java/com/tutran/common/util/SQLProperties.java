...
public final class SQLProperties {
  private static final Logger log = Logger.getLogger(SQLProperties.class);
  private static final String PATH_PREFIX = "sql"; // @MIGR Removed extra file separator 
  ...
  private SQLProperties(String fileName) {
    this(PATH_PREFIX, fileName);    
  }
  
  private SQLProperties(String modeDir, String fileName) {
   ...
   */
  public static SQLProperties getInstance(String fileName) {
    SQLProperties instance = (SQLProperties) cache.get(fileName);
    if (instance == null) {
      instance = new SQLProperties(fileName);
   ...
   * @return instance of SQLProperties
   */
  public static SQLProperties getInstance(String modeDir, String fileName) {
    SQLProperties instance = (SQLProperties) cache.get(fileName);
    if (instance == null) {
      instance = new SQLProperties(modeDir, fileName);
      cache.put(fileName, instance);
    }
    return instance;
  }  
